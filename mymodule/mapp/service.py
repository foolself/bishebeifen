...
List exported services with no permissions required to interact with it:
        ...
        try:
            if arguments.package == None:
                for package in self.packageManager().getPackages(common.PackageManager.GET_SERVICES | common.PackageManager.GET_PERMISSIONS):
                    self.__get_services(arguments, package, opHlr)
            else:
    ...
    def __get_services(self, arguments, package, opHlr):
        ...
        node_service = opHlr.insert("Service")

        if len(exported_services) > 0 or arguments.unexported and len(services) > 0:
            self.stdout.write("Package: %s\n"%package.packageName)

            if not arguments.unexported:
                node_exported = opHlr.insert("Exported_Services", None, node_service)
                for service in exported_services:
                    self.__print_service(package, service, "  ", opHlr, node_exported, arguments.show_intent_filters)
            else:
                self.stdout.write("  Exported Services:\n")
                node_exported = opHlr.insert("Exported_Services", None, node_service)
                for service in exported_services:
                    self.__print_service(package, service, "    ", opHlr, node_exported, arguments.show_intent_filters)
                self.stdout.write("  Hindden Services:\n")
                node_hidden = opHlr.insert("Hidden_Services", None, node_service)
                for service in hidden_services:
                    self.__print_service(package, service, "    ", opHlr, node_hidden, arguments.show_intent_filters)
            self.stdout.write("\n")
        elif arguments.package or arguments.verbose:
            self.stdout.write("Package: %s\n" % package.packageName)
            self.stdout.write("  No exported services.\n\n")

    def __print_service(self, package, service, prefix, opHlr, node_parent, include_intent_filters=False):
        self.stdout.write("%s%s\n" % (prefix, service.name))
        node_item = opHlr.insert("Service", None, node_parent)
        opHlr.insert("Service_Name", service.name, node_item)
        ...
        opHlr.insert("Service_Permission", str(service.permission), node_item)
