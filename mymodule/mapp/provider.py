...
Finding content providers that do not require permissions to read/write:
        ...
        try:
            if arguments.package == None:
                for package in self.packageManager().getPackages(common.PackageManager.GET_PROVIDERS | common.PackageManager.GET_URI_PERMISSION_PATTERNS):
                    self.__get_providers(arguments, package, opHlr)
            else:
    ...
    def __get_providers(self, arguments, package, opHlr):
        ...
        if len(exported_providers) > 0 or arguments.unexported and len(providers) > 0:
            ...
            if not arguments.unexported:
                ...
                node_hidden = opHlr.insert("Hidden_Providers", None, node_provider)
                for provider in hidden_providers:
                    for authority in provider.authority.split(";"):
                        self.__print_provider(provider, authority, "    ", opHlr, node_hidden)
            self.stdout.write("\n")
        elif arguments.package or arguments.verbose:
            self.stdout.write("Package: %s\n" % package.packageName)
            self.stdout.write("  No matching providers.\n\n")

    def __print_provider(self, provider, authority, prefix, opHlr, node_parent):
        ...
        node_item = opHlr.insert("provider", None, node_parent)
        opHlr.insert("Authority", authority, node_item)
        opHlr.insert("Read_Permission", str(provider.readPermission), node_item)
        opHlr.insert("Write_Permission", str(provider.writePermission), node_item)
        opHlr.insert("Content_Provider", str(provider.name), node_item)
        opHlr.insert("Multiprocess_Allowed", str(provider.multiprocess), node_item)
        opHlr.insert("Grant_Uri_Permissions", str(provider.grantUriPermissions), node_item)
