...
class Info(Module, common.Filters, common.IntentFilter, common.PackageManager):
        ...
        try:
            if arguments.package == None:
                for package in self.packageManager().getPackages(common.PackageManager.GET_RECEIVERS | common.PackageManager.GET_PERMISSIONS):
                    self.__get_receivers(arguments, package, opHlr)
            else:
    ...
    def __get_receivers(self, arguments, package, opHlr):
        ...
        node_broadcast = opHlr.insert("Broadcast")

        if len(exported_receivers) > 0 or arguments.unexported and len(receivers) > 0:
            self.stdout.write("Package: %s\n" % package.packageName)

            if not arguments.unexported:
                node_exported = opHlr.insert("Exported_Receivers", None, node_broadcast)
                for receiver in exported_receivers:
                    self.__print_receiver(package, receiver, "  ", opHlr, node_exported, arguments.show_intent_filters)
            else:
                ...
                node_hidden = opHlr.insert("Hidden_Receivers", None, node_broadcast)
                for receiver in hidden_receivers:
                    self.__print_receiver(package, receiver, "    ", opHlr, node_hidden, arguments.show_intent_filters)
            self.stdout.write("\n")
        elif arguments.package or arguments.verbose:
            self.stdout.write("Package: %s\n" % package.packageName)
            self.stdout.write("  No matching receivers.\n\n")

    def __print_receiver(self, package, receiver, prefix, opHlr, node_parent, include_intent_filters=False):
        self.stdout.write("%s%s\n" % (prefix, receiver.name))
        node_item = opHlr.insert("Receiver", None, node_parent)
        opHlr.insert("Receiver_Name", receiver.name, node_item)
        ...
        opHlr.insert("Receiver_Permission", str(receiver.permission), node_item)
class Send(Module):
