...
class Info(Module, common.Filters, common.IntentFilter, common.PackageManager):
    ...
    def execute(self, arguments):
        ...
        try:
            ...
            self.stdout.write(e)
            return 0
    def __get_activities(self, arguments, package, opHlr):
        ...
        node_activity = opHlr.insert("Activity")

        if len(exported_activities) > 0 or arguments.unexported and len(activities) > 0:
            self.stdout.write("Package: %s\n" % package.packageName)

            if not arguments.unexported:
                node_exported = opHlr.insert("Exported_Activity", None, node_activity)
                for activity in exported_activities:
                    self.__print_activity(package, activity, "  ", arguments.show_intent_filters, opHlr, node_exported)
            else:
                ...
                node_hidden = opHlr.insert("Hidden_Activity", None, node_activity)
                for activity in hidden_activities:
                    print "Hidden Activities"
                    self.__print_activity(package, activity, "    ", arguments.show_intent_filters, opHlr, node_hidden)
            self.stdout.write("\n")
        elif arguments.package or arguments.verbose:
            self.stdout.write("Package: %s\n" % package.packageName)
            self.stdout.write("  No matching activities.\n\n")
            
    def __print_activity(self, package, activity, prefix, include_intent_filters, opHlr, node_parent):
        self.stdout.write("%s%s\n" % (prefix, activity.name))
        node_item = opHlr.insert("Activity", None, node_parent)
        opHlr.insert("Activity_Name", activity.name, node_item)
        ...
        opHlr.insert("Activity_Permission", str(activity.permission), node_item)
        if(activity.targetActivity != None):
            self.stdout.write("%s  Target Activity: %s\n" % (prefix, activity.targetActivity))
            opHlr.insert("Target_Activity", activity.targetActivity, node_item)
